package org.abondar.industrial.parkinglot.util;

/**
 *
 * List output messages
 */
public class MessagesUtil {

    public static final String CREATED_LOT_MSG = "Created a parking lot with %d slots\n";
    public static final String ALLOCATED_MSG = "Allocated slot number: %d\n";
    public static final String LOT_FULL_MSG = "Sorry, parking lot is full\n";
    public static final String SLOT_NOT_FOUND = "%d not found\n";
    public static final String PARK_MSG = "park %s %s\n";

}
